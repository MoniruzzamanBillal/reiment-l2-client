...
type TReview = {
  ...
  rating: number;
  user: { email: string; id: string; profileImg: string; username: string };
};

const RecentReview = () => {
  const { data: recentReviews, isLoading } = useFetchData<TReview[]>(
    ["recentReview"],
    "/review/recent-review"
  );

  const reviews: TReview[] = (recentReviews as any)?.data ?? [];
