...
export default function TextEditorTipTap({
  ...
  const editor = useEditor({
    ...
    extensions: [
      StarterKit.configure({
        ...
        codeBlock: {
          HTMLAttributes: {
            class: "bg-surface-primary text-surface-text border border-surface-border p-4 rounded",
          },
        },
      }),
  ...
  if (!editor) {
    return (
      <div className={`border rounded-lg animate-pulse min-h-75 ${className}`}>
        <div className="h-10 bg-surface-secondary rounded-t-lg"></div>
        <div className="p-4 bg-surface-primary rounded-b-lg"></div>
      </div>
    );
  }
  ...
  return (
    <div className={`border rounded-lg shadow-sm ${className}`}>
      ...
      {/* Editor Content */}
      <div className="min-h-75 max-h-140 overflow-y-auto bg-surface-primary rounded-b-lg">
        <EditorContent editor={editor} />
      </div>

      {/* Character Count */}
      <div className="px-4 py-2 border-t border-surface-border text-sm text-surface-text-muted bg-surface-secondary rounded-b-lg">
