...
7. **[07: Deployment Verification & README Polish](./07-deployment-readme-polish.md)** — verify the existing live Vercel deployment end-to-end and add a demo link/screenshots/architecture summary to `README.md`. Docs + manual verification only. Plan written, awaiting user review/approval before implementation.
8. **[08: Frontend Testing](./08-frontend-testing.md)** — Vitest + React Testing Library unit/component tests plus Playwright E2E for the golden paths. No test suite exists today. Plan written, awaiting user review/approval before implementation.
9. **[09: Frontend CI Pipeline](./09-ci-pipeline-frontend.md)** — GitHub Actions lint/build/test workflow. Depends on spec `08` landing first. Plan written, awaiting user review/approval before implementation.
10. **[10: Real-Time Order Notifications (Frontend)](./10-realtime-notifications-frontend.md)** — Pusher client for live vendor/customer order updates. Candidate "depth" feature (alternative: server spec `07`). Depends on the backend plan (server spec `06`) landing first. Plan written, awaiting user review/approval before implementation.
11. **[11: Frontend Code-Quality Cleanup](./11-code-quality-cleanup-frontend.md)** — remove scattered `any` typing in mutation call sites, unify the two breakpoint-naming conventions in `globals.css`. Plan written, awaiting user review/approval before implementation.
12. **[12: Dockerize the Client](./12-dockerization.md)** — multi-stage `Dockerfile` for the Next.js client, coordinating with the server repo's `docker-compose.yml` (spec `09` there). Plan written, awaiting user review/approval before implementation.
13. **[13: Low-Stock Inventory Notifications (Frontend)](./13-low-stock-inventory-notifications-frontend.md)** — bind a `low-stock` event on the existing vendor Pusher channel (`useOrderPusher.ts`) to a toast + product-list refetch. Depends on the backend plan (server spec `11`) landing first. Implemented.
